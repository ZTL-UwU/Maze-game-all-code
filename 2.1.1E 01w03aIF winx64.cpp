...
s _mcheck;
s _dcheck;
long long coins; // vis in the shop
short shortest = 0;
...
void feedback (){
	system("cls");
	cout << "Any question?\n";
	cout << "Please contact Tianli Zhang";
	sleep (3);
	return;
}
...
int main (){
	...
	while (last == 6){
		...
		system("color F0");
		while (true){
			cout << "<easy> (e) , <middle> (m) , <hard> (h) , <crazy> (c) , <random> (r) , <back> (b)";
       		hard = getch();
        	system("cls");
		...
		while (true){
			cout << "Choose a level: 1,2,3,4,5,6,7,8,9 , <back> (b)" << endl;
        	temp_char = getch();
			if (temp_char == 'b'){
				goto Main;
			}
