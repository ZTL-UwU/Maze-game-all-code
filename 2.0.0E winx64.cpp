...
s _check;
long long coins; // used in the shop
short shortest = 0;
short end = 6; // check if the game is end
...
void playerlevel_show (){
	system("cls");
	system("color A0");
	cout << "<now level>: " << playerlevel_short << endl;
...
void rule (){
    ...
    system("cls");
	printf("rules:\n");
	printf("w or ¡ü for up, a or ¡û for left, s or ¡ý for down, d or ¡ú for right.\n");
...
int main (){
	...
	while (end == 6){
		while (true){
			system("cls");
			system("color F0");
			cout << "<shop> (s) , <player level> (p) , <continue> (c) , <leave> (l) , <rule> (r)" << endl;
		...
		system("color F0");
		while (true){
			cout << "<easy> (e) , <middle> (m) , <hard> (h) , <crazy> (c)";
       		hard = getch();
        	system("cls");
        	if (hard != 'e' && hard != 'm' && hard != 'h' && hard != 'c'){
		...
		while (true){
			cout << "Choose a level: 1,2,3,4,5,6,7,8,9" << endl;
        	temp_char = getch();
        	level = temp_char - '0';
        	system("cls");
        	if (level < 1 || level > 10){
		...
		endlevel << playerlevel_short;
       	end = MessageBox(GetForegroundWindow(),"Play again?","",4);
        system ("cls");
    	x = 1;
    	y = 1;
    	over = 3;
