...
char temp[50][50];
char skin = 'O';
char version[] = "2.1.2E 03w01aIF winx64";
char sound_play[100];
...
void SetPointerPos(int y, int x)
{
	COORD pos;
	pos.X = x;
	pos.Y = y;
...
void rule()
{
	...
	printf("@ is a machine to go to another @.\n");
...
//		putchar(' ');
		short xx = chang + 4 + 3;
		short chang2 = chang - 12;
		if (chang2 < 0) chang2 = 0; 
		if (!showrecord) xx --;
		SetPointerPos(xx, chang2 / 2 + 12);
		printf("%d", c + 1);
		SetPointerPos(2 + x, 3 + y);
		putchar(' ');
		x--;
		over = check(x, y);
		SetPointerPos(2 + x, 3 + y);
...
int BFS()
{
	...
	while (!q.empty())
	{
		short x = q.front().x;
		short y = q.front().y;
		short step = q.front().step;
		tmpstep = step;
		if (x == chang - 1 || y == chang - 1)
			return tmpstep;
		q.pop();
		if (x + 1 < chang && !vis[x + 1][y] && temp[x + 1][y] == ' ')
		{
			q.push((data){x + 1, y, step + 1});
			vis[x + 1][y] = true;
		}
		if (x - 1 >= 0 && !vis[x - 1][y] && temp[x - 1][y] == ' ')
		{
			q.push((data){x - 1, y, step + 1});
			vis[x - 1][y] = true;
		}
		if (y + 1 < chang && !vis[x][y + 1] && temp[x][y + 1] == ' ')
		{
			q.push((data){x, y + 1, step + 1});
			vis[x][y + 1] = true;
		}
		if (y - 1 >= 0 && !vis[x][y - 1] && temp[x][y - 1] == ' ')
		{
			q.push((data){x, y - 1, step + 1});
			vis[x][y - 1] = true;
		}
...
void random()
{
	...
	while (true)
	{
		...
		crazy:
			...
			if (temp_int >= 40)
			{
				MessageBox(GetForegroundWindow(), "map size must < 40", "", 4);
				goto crazy;
			}
			chang = temp_int;
		...
		else if (temp_char == 'l' || temp_char == 's')
		{
			if (temp_char == 'l')
			{
